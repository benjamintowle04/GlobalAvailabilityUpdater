...
def remove_class_times(available_times, start, end):
    return [
        time for time in available_times if not (start <= time_to_minutes(time) < end)
    ]
...
def process_class_schedule(available_times_per_day, class_schedule):
    ...
    for class_info in class_schedule:
        start_time_minutes = time_to_minutes(
            datetime.strptime(class_info["start"], "%I:%M:%S %p").strftime("%H:%M")
        )
        end_time_minutes = time_to_minutes(
            datetime.strptime(class_info["end"], "%I:%M:%S %p").strftime("%H:%M")
        )
        ...
        for day in meeting_days:
            ...
            available_times_per_day[day_name] = remove_class_times(
                available_times_per_day[day_name], start_time_minutes, end_time_minutes
            )
...
def condense_available_times_per_day(available_times_per_day):
    return {
        day: condense_times(times) for day, times in available_times_per_day.items()
    }
...
def getAvailableRanges(employee_id):
    ...
    available_times_per_day = process_class_schedule(
        available_times_per_day, employee_classSchedule
    )
    # Condense available times into ranges for each day
    condensed_available_times_per_day = condense_available_times_per_day(
        available_times_per_day
    )
